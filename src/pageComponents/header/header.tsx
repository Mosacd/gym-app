    ...
    <header className={headerClass()}>
      <div className={containerClass()}>
        ...
        <div className={rightSectionClass()}>
          ...
          { user ?   (<Link to={"/dashboard/profilePage"}><Avatar className='h-10 w-10 border-4 border-purple-900' >
                  <AvatarImage src={profileData?.avatar_url} />
                  <AvatarFallback>CN</AvatarFallback>
                </Avatar>
                </Link>
                ) : <Link to={"/auth/signin"}><Button className={loginButtonClass()}>{Icons.man} Log in</Button></Link>
          }
