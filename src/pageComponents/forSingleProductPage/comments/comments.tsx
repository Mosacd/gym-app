...
import { useGetLikedByUser, useGetProductReviews } from "@/reactQuery/query/reviews";
...
const { data: likeDataQ, isLoading } = useGetLikedByUser({reviewId: selectedReviewId || undefined ,userId: user?.id});


const {mutate:likefn,  isPending:likePending} = useToggleLike()

  const helpfulReview = (userId: string, reviewId: number, productId:string) => {
                        ...
                        <div className="flex flex-col items-center sm:items-end lg:items-center lg:flex-row lg:gap-14">
                          <div className="flex items-center">
                            <span className="text-sm text-black font-semibold dark:text-white">
                              {reviews[row.index].like_count} people found this review helpful
                            </span>
                          </div>
                          <div className="flex items-center">
      ...
      <DialogHeader>
        <DialogTitle className="text-3xl dark:text-neutral-400 text-center">
          <div>
            <div className="flex justify-center items-center gap-2">
              ...
              />
              <span>
                {selectedReview.profiles.username}'s Review
              </span>
            </div>
            <span className="text-sm text-gray-500">
        ...
        <DialogDescription className="w-full">
                      <div className="w-full text-lg text-center mt-10 text-black dark:text-gray-400">
                        <div className="break-words break-all overflow-y-auto max-h-72  sm:max-h-96"
                        ...
                        <div className="flex flex-col sm:flex-row justify-between items-center mt-10">
                          <div className="flex flex-col">
                            ...
                            <div className="text-base font-semibold text-gray-600">
                              {selectedReview.like_count} people found
                              this review helpful
                            </div>
                          </div>
                          <div className="flex flex-col items-center gap-2">
                           ...
                           {likeDataQ?.liked ? (  <Button
          onClick={() => helpfulReview(user?.id || "", selectedReview.id, selectedReview.product_id?.toString() || "")}
                               
                                className={`flex items-center ${likePending ? 'opacity-70 cursor-not-allowed' : ''}`} disabled={likePending}
                                variant={"secondary"}
                              >
                                <span>No</span>
                                <svg
                          ...
                          />
                              ...
                              <Button className={`flex items-center ${likePending ? 'opacity-70 cursor-not-allowed' : ''}`} disabled={likePending} onClick={() => helpfulReview(user?.id || "", selectedReview.id, selectedReview.product_id?.toString() || "")}>
                              <span>Yes</span>
                                                    <svg
