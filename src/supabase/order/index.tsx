...
export const placeOrder = async ({
    ...
    if (!userId || !items || items.length === 0 || !totalPrice) {
      throw new Error("User ID, items, and total price are required to place an order.");
    }
   ...
   export const getUserOrders = async (userId: string | undefined):Promise<Order[]> => {
    if(userId === undefined){ throw new Error("User ID is required to fetch orders.")}
